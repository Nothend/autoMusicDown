...
echo "[$(date +"%Y-%m-%d %H:%M:%S %Z")] 配置代码拉取定时任务：$PULL_CRON"
echo "[$(date +"%Y-%m-%d %H:%M:%S %Z")] 配置程序执行定时任务：$CRON_SCHEDULE"
...
if ! echo "$CRON_JOBS" | crontab -; then
    ...
    echo "[$(date +"%Y-%m-%d %H:%M:%S %Z")] 错误：写入crontab配置失败！" >&2
    echo "[$(date +"%Y-%m-%d %H:%M:%S %Z")] 失败的crontab配置内容：" >&2
    echo "$CRON_JOBS" >&2  # 输出具体配置内容，方便排查格式问题
    echo "[$(date +"%Y-%m-%d %H:%M:%S %Z")] 请检查cron表达式格式或系统权限" >&2
    exit 1  # 失败时退出脚本，避免容器启动后定时任务无效
fi

# 启动 cron 服务
echo "[$(date +"%Y-%m-%d %H:%M:%S %Z")] 启动定时任务服务..."
exec crond -f -l 2